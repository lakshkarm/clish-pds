...
## Assign a list of snapshot/clones to this methode to assign then to the controller port 
    def assignCopy(self,typ,copylist,port1,port2):
        ...
        for i in copylist:
            cmd = "assign copy type " +str(typ)+ " name " +str(i)+" port-name "+str(port1)+","+str(port2) 
    ...
    def assignVol(self,vollist,port1,port2):
        clishObj = self.psshObj()
        for i in vollist:
            cmd = "assign volume name " + str(i)+ " port-name "+str(port1)+","+str(port2)
            clishObj.sendline(cmd)
            clishObj.prompt()
        print(clishObj.before)
        clishObj.sendline('exit')
    ...
    def unassignCopy(self,typ,copylist):
        clishObj = self.psshObj()
        for i in copylist:
            cmd = "unassign copy type "+str(typ)+" name "+str(i)
            clishObj.sendline(cmd)
            clishObj.prompt()
        print(clishObj.before)
...
# starting to create snapshots here 
    for i in range(10):
        snaplist = obj.createSnap("mvol1",7)
        #obj.assignCopy("snapshot",snaplist,"40g-10/3","40g-6/3")
        #obj.assignCopy("snapshot",snaplist,"40g-6/1","40g-10/2")
        #obj.unassignCopy("snapshot",snaplist)
        obj.deleteSnap(snaplist)
        print "Completed iteration : ",i
        #print "%sst iteration completed successfully\n"%i
    #obj.deleteSnap(snaplist)
